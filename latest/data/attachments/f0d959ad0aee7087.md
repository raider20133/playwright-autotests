...
- Name: tests/api/tasks.spec.ts >> Tasks API >> known bugs: database constraints leak as 500 >> a negative price should be a 400, not a 500
- Location: tests/api/tasks.spec.ts:47:13

# Error details

```
Error: POST https://managmenttool.onrender.com/api/tasks → 500 {"message":"Database error creating task","error":"new row for relation \"tasks\" violates check constraint \"tasks_price_check\""}
